...
public class ConfigurationMergeHandler {
     ...
     */
    public NodeSet updateConfiguration(final NodeSet masterConfiguration, final NodeSet configurationFragment,
                                       final String xPath) {
        ...
        final Element fragmentElementToBeAdded =
                identifySubFragmentToBeAdded(configurationFragment, pathComponents, masterElementDepth);
    ...
    private Element identifySubFragmentToBeAdded(final NodeSet fragment, final List<PathComponent> pathComponents,
        ...
        for (final PathComponent component : pathComponents) {
            ...
            for (final Element candidateElement : candidates) {
                if (isMatchingElement(candidateElement, component)) {
                    //                    LOG.debug(">>>Matching Elements:{}--{}", candidateElement.toXMLString(),
                    // component);
    ...
    private boolean isMatchingElement(final Element elem, final PathComponent pc) {
        return elem != null && pc != null && elem.name.equals(pc.name) && areMatchingKeys(elem.getChildren(), pc.keys);
    }
    ...
    private boolean areMatchingKeys(final NodeSet keyElements, final List<Key> keys) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            if (!keyElements.getElement(i).name.equals(keys.get(i).name) || !keyElements.getElement(i).getValue()
                    .toString().equals(keys.get(i).value)) {
                return false;
            }
        }
        return true;
