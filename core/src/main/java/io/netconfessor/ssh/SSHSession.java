...
class SSHSession implements Transport {

    private static final String END_MARKER = "]]>]]>";
    private static final int END = END_MARKER.length() - 1;
     ...
     */
    SSHSession(SSHConnection con) throws IOException, JNCException {
        this(con, 0);
    }
     ...
     */
    SSHSession(SSHConnection con, long readTimeout) throws IOException, JNCException {
    ...
    public boolean serverSideClosed() throws IOException {
        int conditionSet = ChannelCondition.TIMEOUT & ChannelCondition.CLOSED & ChannelCondition.EOF;
    ...
    public StringBuffer readOne() throws IOException, JNCException {
        ...
        while (true) {
            if ((readTimeout > 0) && !in.ready()) { // else we want to block
                final int conditionSet = session.waitForCondition(0xffffffff, readTimeout);
                if ((conditionSet & ChannelCondition.TIMEOUT) == ChannelCondition.TIMEOUT) {
                    // it's a timeout - there is nothing to
                    // read, not even eof
                    throw new JNCException(JNCException.TIMEOUT_ERROR, Long.valueOf(readTimeout));
                }
            }
            ...
            for (int i = 0; i < END_MARKER.length(); i++) {
                if (ch == END_MARKER.charAt(i)) {
                    if (i < END) {
                        ch = in.read();
                    } else {
                        for (final IOSubscriber sub : ioSubscribers) {
                            sub.inputFlush(END_MARKER.substring(0, END));
                        }
                        return wr.getBuffer();
                    }
                } else {
                    subInputChar(wr, END_MARKER.substring(0, i));
                    subInputChar(wr, ch);
                    break;
                }
            }
     ...
     */
    public void flush() {
        out.print(END_MARKER);
        out.flush();
        for (final IOSubscriber sub : ioSubscribers) {
            sub.outputFlush(END_MARKER);
        }
    }
