...
public class Device implements Serializable {
     ...
     */
    public Device(TransportFactory transportFactory, String name, DeviceUser user, String mgmtIp, int mgmtPort) {
     ...
     */
    public Device(TransportFactory transportFactory, String name, String mgmtIp, int mgmtPort) {
    ...
    public void setReadTimeout(String sessionName, int readTimeout) {
        ...
        p.transport.setReadTimeout(readTimeout);
    }
    ...
    public long getReadTimeout(String sessionName) {
        final SessionConnData p = getConnData(sessionName);
        return p.transport.getReadTimeout();
    }
     ...
     */
    public Transport getTransportSession(String sessionName) {
        final SessionConnData data = getConnData(sessionName);
        return (data == null) ? null : data.transport;
    }
     ...
     */
    public void connect(String localUser, int connectTimeout) throws IOException, JNCException {
        DeviceUser u = null;
        ...
        if (u == null) {
            throw new JNCException(JNCException.AUTH_FAILED, "No such user: " + localUser);
        }
        transportFactory.initialize(mgmtIp, mgmtPort, connectTimeout);
        transportFactory.auth(u);
    }
    ...
    public void newSession(IOSubscriber sub, String sessionName) throws JNCException, IOException {
        ...
        final Transport transport = transportFactory.createTransport();
        if (sub != null) {
            transport.addSubscriber(sub);
        }
        final NetconfSession session = new NetconfSession(transport, parser);
        final SessionConnData d = new SessionConnData(sessionName, transport, session);
        connSessions.add(d);
    ...
    public void runBacklog(String sessionName) throws IOException, JNCException {
        ...
        final SessionConnData data = getConnData(sessionName);
        if (data == null) {
            throw new YangException(YangException.BAD_SESSION_NAME, sessionName);
        }
    ...
    public String toString() {
        StringBuffer s = new StringBuffer("Device: " + name + " " + mgmtIp + ":" + mgmtPort + "\n");
    ...
    private static class SessionConnData {
        ...
        SessionConnData(String n, Transport t, NetconfSession s) {
            sessionName = n;
            session = s;
            transport = t;
        }
