...
public class Path {
     ...
     */
    protected ArrayList<LocationStep> locationSteps;
    /**
    ...
    NodeSet evalStep(NodeSet nodeSet, int step) throws JNCException {
        if (step < 0 || step >= locationSteps.size()) {
            throw new JNCException(JNCException.PATH_ERROR, "cannot eval location step: " + step + " in path");
        }
        ...
        try {
            ...
            while (sz > 0) {
                ...
                if (tok1.type == SLASH) {
                    ...
                    steps.add(step);
                    tokens.remove(0);
                } else if (tok1.type == ATOM  /* AXIS::PREFIX:TAG */ && tok2.type == COLONCOLON && tok3.type == ATOM
                    ...
                    parsePredicates(tokens, step);
                } else if (tok1.type == ATOM && tok2.type == COLON && tok3.type == ATOM) { /* PREFIX:TAG */
                    step = new LocationStep(AXIS_CHILD, tok1.value, tok3.value);
                    steps.add(step);
                    tokens.removeRange(0, 3);
                    parsePredicates(tokens, step);
                } else if (tok1.type == ATOM) { /* TAG */
                    step = new LocationStep(AXIS_CHILD, tok1.value);
                    steps.add(step);
                    tokens.remove(0);
                    parsePredicates(tokens, step);
                } else if (tok1.type == SLASHSLASH) { /* '//' (SLASHSLASH) */
    ...
    int parseAxis(String str) throws JNCException {
        ...
        throw new JNCException(JNCException.PATH_ERROR, "unsupported or unknown axis: " + str);
    }

    /**
     * parse out predicates
     */
    void parsePredicates(TokenList tokens, LocationStep step) throws JNCException {
        ...
        if (sz >= 1) {
            ...
            if (tok1.type == LPRED) {
                ...
                try {
                    ...
                    throw new JNCException(JNCException.PATH_ERROR, "unmatched '[' in expression");
                }
    ...
    Expr parsePredicate(TokenList tokens, int from, int to) throws JNCException {
        ...
        while (i < to) {
            ...
            trace("parsePredicate(): from=" + from + " to=" + to + " [" + tok1 + "," + tok2 + "," + tok3 + ", ...]");
            ...
            } else if (tok1.type == ATTR && (tok2 != null ? tok2.type : 0) == COMPARE && tok3 != null) {
                final Object rexpr = parsePredicateRvalue(tokens, i + 2, to);
     ...
     * from 'from' to 'to'.
     */
    Object parsePredicateRvalue(TokenList tokens, int from, int to) throws JNCException {
        Token tok1;
        Token tok2;
        Token tok3;
        final int i = from;
        while (i < to) {
            ...
            trace("parsePredicateRvalue(): from=" + from + " to=" + to + " [" + tok1 + "," + tok2 + "," + tok3
                    + ", ...]");
    ...
    TokenList tokenize(String s) throws JNCException {
        ...
        while (i < s.length()) {
            ...
            if (curr == ' ' || curr == '\t' || curr == '\n') {
                ...
                boolean escape = (curr == '\\');
                j = i + 1;
                while (j < buf.length && ((buf[j] >= 'a' && buf[j] <= 'z') || (buf[j] >= 'A' && buf[j] <= 'Z') || (
                ...
                i = j;
            } else if (curr == '@' && ((next >= 'a' && next <= 'z') || next >= 'A' && next <= 'Z')) {
                i++;
                j = i + 1;
                while (j < buf.length && ((buf[j] >= 'a' && buf[j] <= 'z') || (buf[j] >= 'A' && buf[j] <= 'Z') || (
                        buf[j] >= '0' && buf[j] <= '9') || (buf[j] == '-') || (buf[j] == '_'))) {
                    j++;
                }
                ...
                String value;
                Number number;
                if ((j + 1) < buf.length && buf[j] == '.' && (buf[j + 1] >= '0' && buf[j + 1] <= '9')) { // float
                    j++;
                    while (j < buf.length && (buf[j] >= '0' && buf[j] <= '9')) {
                        j++;
                    }
                ...
                switch (curr) {
                    ...
                    case '<':
                        ...
                        break;
                    default:
                        throw new JNCException(JNCException.PATH_ERROR, "illegal character in expression: " + curr);
                }
                i++;
            }
        }
    ...
    class LocationStep {
        ...
        private ArrayList<Expr> predicates; // list of Expr (Predicates)
         ...
         */
        Element createElem(PrefixMap prefixMap, Element parent) throws JNCException {
            ...
            switch (axis) {
                ...
                case AXIS_PARENT:
                default:
                    throw new JNCException(JNCException.PATH_CREATE_ERROR, "unknown axis in create path");
            }
        }
    ...
    class Expr {
        ...
        private int op; // See OP codes above
        private Object lvalue; // Expr|Boolean|String|Integer|NodeSet
        private Object rvalue; // Expr|Boolean|String|Integer|NodeSet
        ...
        public Boolean eval(Element node, NodeSet contextSet) throws JNCException {
            return fBoolean(eval2(node, contextSet));
        }

        private Object eval2(Element node, NodeSet contextSet) throws JNCException {
            Object lval; // results
            Object rval;
            lval = lvalue;
            rval = rvalue;
            ...
            switch (op) {
                ...
                default:
                    throw new JNCException(JNCException.PATH_ERROR, "illegal operator: " + op);
            }
        }
        ...
        private int compare(Object x, Object y) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to compare (not of same type): " + x + ", " + y);
        }
        ...
        private Boolean fBoolean(Object x) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to function boolean(): " + x);
        }
        ...
        private Number fNumber(Object x) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to function number(): " + x);
        }
        ...
        private NodeSet fNodeSet(Object x) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to function nodeset(): " + x);
        }
        ...
        private Number neg(Object x) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to function neg(): " + x);
        }
        ...
        private Number minus(Number x, Number y) throws JNCException {
            if ((x instanceof Integer) && (y instanceof Integer)) {
                return Integer.valueOf(((Integer) x).intValue() - ((Integer) y).intValue());
            }
        ...
        private Float fFloat(Object x) throws JNCException {
            ...
            throw new JNCException(JNCException.PATH_ERROR, "badarg to function float(): " + x);
        }
        ...
        Object evalCreate(Element node) throws JNCException {
            ...
            switch (op) {
                ...
                case EQ: // '='
                    ...
                    throw new JNCException(JNCException.PATH_CREATE_ERROR, "illegal path create expr: " + this);
                default:
                    throw new JNCException(JNCException.PATH_CREATE_ERROR, "illegal path create expr:" + this);
            }
        }
