 ...
 */
public abstract class YangBaseInt<T extends Number> extends YangBaseType<T> {
     ...
     */
    protected void setMinMax(Number minValue, Number maxValue) throws YangException {
        this.minValue = Utils.bigDecimalValueOf(minValue);
        this.maxValue = Utils.bigDecimalValueOf(maxValue);
        if (this.minValue == null || this.maxValue == null) {
            return;
        }
        YangException.throwException(this.minValue.compareTo(this.maxValue) > 0, this);
    }
    ...
    protected boolean valid(Number n) {
        ...
        boolean res = true;
        if (minValue != null) {
            res = bd.compareTo(minValue) >= 0;
        }
        if (maxValue != null) {
            res &= bd.compareTo(maxValue) <= 0;
        }
        return res;
    }
