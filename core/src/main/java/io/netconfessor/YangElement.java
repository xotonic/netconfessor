 ...
 * {@link Element Element}. It is an extension of the Element
...
public abstract class YangElement extends Element {
    ...
    protected static final String DUMMY_LC = "dummy";
    ...
    private static final List<String> OTHER_RESERVED_WORDS = Arrays.asList("string", "value");
    private static final String COLON_UNEXPECTED_ELEMENT = ": Unexpected element";
    /**
    ...
    private static Element instantiate(Element parent, String name, String pkg)
        ...
        final String className = pkg + "." + getPackage(parent) + normalize(name);
     ...
     */
    public static Element createInstance(ElementHandler parser, Element parent, String ns, String name)
        ...
        try {
            ...
            throw new YangException(YangException.ELEMENT_MISSING, name);
        }
    }
    ...
    private static boolean isReserved(String s) {
        ...
        final boolean isAnotherReservedWord = OTHER_RESERVED_WORDS.contains(s);
        return isAnotherReservedWord;
    }
     ...
     */
    public static void getDiff(YangElement a, YangElement b, NodeSet uniqueA, NodeSet uniqueB, NodeSet changedA,
                               NodeSet changedB) {
        if (a.compare(b) >= 0) {
            ...
            for (int i = 0; i < a.children.size(); i++) {
                ...
                if (bRes >= 0) {
                    ...
                    if (bRes == 1) { // different content
                        ...
                        // bRes == 0 or 2 so they are equal, but their children
                        // might not be, so we recurse
                        getDiff((YangElement) aChild, (YangElement) bChild, uniqueA, uniqueB, changedA, changedB);
                    }
    ...
    private static int csync2(YangElement a, YangElement b, NodeSet toDel) {
        ...
        for (int i = 0; b.children != null && i < b.children.size(); i++) {
            ...
            if (aChild instanceof YangElement) {
                final int d = YangElement.csync2((YangElement) aChild, (YangElement) bChild, toDel);
                diffs += d;
                if (d == 0) {
        ...
        for (int i = 0; a.children != null && i < a.children.size(); i++) {
            ...
            if (x instanceof Leaf) {
                ...
                b.addChild(x);
                x.markDelete();
            } else if (x instanceof YangElement) { // Remove all children except keys (if any)
                diffs++;
    ...
    public static NodeSet sync(NodeSet a, NodeSet b) throws JNCException {
        ...
        if (result != null) {
            NodeSet editConfig = result.getChildren();
            // remove dummy from parents
            for (Element e : editConfig) {
                e.parent = null;
            }
            return editConfig;
        } else {
     ...
     * @throws JNCException
     */
    public void setLeafValue(String ns, String name, String value) throws YangException, JNCException {

        // Aware
        final String methodName = "instantiateValue";
        final Class<?>[] types = new Class[] {String.class};
        try {
            Method setLeafValue = getClass().getMethod(methodName, types);
            setLeafValue.invoke(this, value);
        } catch (final NoSuchMethodException e) {
            try {
                final String leafListMethodName = "set" + normalize(name) + "Value";
                Method setLeafValue = getClass().getMethod(leafListMethodName, types);
                setLeafValue.invoke(this, value);
            ...
            // type error
            throw new YangException(YangException.BAD_VALUE,
                    getElementPath(name) + ": " + invErr.getCause().toString());
        }
    }

    protected void setLeafValue(String ns, String path, Object value, String[] childrenNames) throws JNCException {
    ...
    protected void setAnyxmlValue(String ns, String path, Object value, String[] childrenNames) throws JNCException {
        final NodeSet nodes = get(path);

        if (nodes.isEmpty()) {
            final Element leaf = new Element(ns, path);
            insertChild(leaf, childrenNames);
        }
    }

    protected Leaf setLeafListValue(String ns, String path, Object value, String[] childrenNames) throws JNCException {
        final Element listEntry = get(path).last();

        if (listEntry instanceof Leaf && listEntry.value == null) {
            listEntry.setValue(value);
            return ((Leaf) listEntry);
        } else {
            final Leaf leaf = new Leaf(ns, path);
            leaf.setValue(value);
            insertChild(leaf, childrenNames);
    ...
    protected void markLeafReplace(String path) throws JNCException {
        ...
        if (nodes.isEmpty()) {
            throw new YangException(YangException.ELEMENT_MISSING, getElementPath(path));
        } else {
    ...
    public boolean keyCompare(Element b) {
    ...
    /**
     * Skip comparing keys and children
     */
    protected int elementCompare(Element b) {
        return super.compare(b);
    }
                            ...
                            case RevisionInfo.R_MAX_ELEM_RAISED:
                                ...
                                if (getChildren().size() > max) {
                                    throw new JNCException(JNCException.REVISION_ERROR,
                                            tp + "too many children for old node " + "with rev( " + rev + ")");
                                }
                                break;
    ...
    public void removeNonKeysIfMarkedToDelete() {
        final Attribute attr = getAttr(OPERATION);
        if (attr == null) {
            return;
        }
        if (DELETE.equals(attr.value) && children != null) {
            final List<Element> notKeys = new ArrayList<>();
            for (Element child : children) {
                if (!child.isKey()) {
                    notKeys.add(child);
                }
            }
            for (Element notKey : notKeys) {
                deleteChild(notKey);
            }
        }
    }

    public boolean containsKeysOnly() {
        final String[] keyNames = keyNames();
        if (keyNames == null) {
            return false; // not list
        }
        if (keyNames.length != children.size()) {
            return false; // children count more than key count or not enough keys
        }
        // now just check that all children is keys
        for (Element child : children) {
            if (!child.isKey()) {
                return false;
            }
