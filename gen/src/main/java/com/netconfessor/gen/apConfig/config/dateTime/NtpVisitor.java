...
import com.netconfessor.gen.apConfig.config.dateTime.ntp.Server;
...
public abstract class NtpVisitor {
    ...
    public void setup() {
        setupServer(new LeafYangData<>("server", "/ap-config:config/ap-config:date-time/ap-config:ntp/ap-config:server", "NTP server address", true, YangDataType.leaf, "com.netconfessor.gen.ietfInetTypes.Host", s -> new Host(s)));
    }
